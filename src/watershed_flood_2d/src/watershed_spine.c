...
//==========================================
void local_watershed_sort_double(double *den, int *voids, char *boundaries, int sizeX, int sizeY, int sizeZ)
{
  int     i,j;
  int     n_shell=27;
  int    *shell_ind   = malloc(n_shell*sizeof(int));
  ...
  //--- Get indexes of boundaries. Actually these are the indexes in void regions, not boundaries
  ...
  for (i=0; i<n_pixels;i++)
	...
	voids[i] = -1;
      ...
      //--- OJO: previous version had watershed=0 and regions started at 1 but it was problematic
      //else
      //{
      //	  voids[i] = voids[i]+1; //--- Add one to void counter since watershed transform=0 and regions can start at 0 too.
      //	}
    }
