...
int *watershed_uphill_27(double *density, int x_grid, int y_grid, int z_grid)
{
  ...
  flood_basin_uphill(closest_minima, voids, x_grid, y_grid, z_grid, n_local_minima, local_minima);
...
int *watershed_uphill_27_float(float *density, int x_grid, int y_grid, int z_grid)
{
  ...
  //--- Find local minima
  //==================================
  local_minima = find_minima_int_27_float(density, x_grid,y_grid,z_grid, &n_local_minima);
  printf(">>> Ready finding minima [%d]\n", n_local_minima);
  fflush(stdout);
  ...
  //--- Get closest (lower) pixel in neighbourhood
  get_closest_float(density, closest_minima, x_grid, y_grid, z_grid);
...
void flood_basin_uphill(int *closest_minima, int *voids, int x_grid, int y_grid, int z_grid, int total_minima, int *local_minima)
{
  ...
  for(i=0; i<total_minima; i++){
    ...
    shell_27(shell_ind, xg,yg,zg, x_grid,y_grid,z_grid);
    ...
    //-----------------------------------------
    recursive_flood_uphill(where_array, closest_minima, voids, Void_Id, x_grid, y_grid, z_grid);
...
void recursive_flood_uphill(int *children, int *closest_minima, int *voids, int ID, int x_grid, int y_grid, int z_grid){
  ...
  for (i=0;i<children[0];i++){
    ...
    //--- Call recursively
    //-----------------------------------------
    recursive_flood_uphill(where_array, closest_minima, voids, ID, x_grid, y_grid, z_grid);
