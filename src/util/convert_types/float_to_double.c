/*
   ...
   COMPILE AS:
       ...
       icc -o double_to_float double_to_float.c io_files.c
...
int main(int argc, char **argv)
{
  ...
  if (argc < 2) {
    printf("------------------------------------------ \n");
    printf(" Usage:\n");
    printf("     ./float_to_double file_double file_float \n");
    printf("\n");
    printf("------------------------------------------ \n");
    exit(0); }
  ...
  for (i=0; i<sizeX; i++) {
    ...
    for (j=0; j<n_buffer; j++) {
      fread(&tempf, sizeof(float), 1, file_inp);
      data_buffer[j] = (double) tempf;
    }
